...
`@deepseek-ai/cordis-plugin-include` parses `!!js` scalars into expression nodes. Loader interpolates `config` (after injections activate, against that plugin context — `ctx.serviceName`) and `disabled` (at every mount decision, against the loader context). Nested row expressions stay until that row activates; other metadata stays literal. Quote any expression that contains `?` or `:` — unquoted they are YAML mapping syntax and the expression never runs. Use overlays when the environment selects plugins.
