...
`createApiRemoteAgentProjection()` is the main-process remote-projection factory for the same identities (the P0 agent-lookup transformation): the durable header/events come from the persistence projection, the live descriptor and a bounded command handle come from `ctx.agentControl` when a control provider is mounted (resuming a durable cold session through the control service), and subagent-owned sessions keep the legacy fence. Without a control provider it is a pure cold read. Consumers that need only the read model, or a command handle without live `Agent` access, use this factory; live-`Agent` consumers (the Web API Proxy) keep `createApiRemoteAgentResolver()` until their implementations move into the worker-local composition.
