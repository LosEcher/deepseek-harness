...
| `bootQuarantiningAddons(binName, absoluteConfigPath, patches, addonIds, prepare?, bareModuleBaseUrl?, warn?)` | 调用 `boot`，禁用每个失败的插件行并重试；若失败点名了非插件行则重新抛出。隔离只存在于内存。profile 启动器通过 `profileAddonEntryIds` 计算 `addonIds` |
| `bundleResolvesFromInstallation` / `collectAddonEntryIds` / `profileAddonEntryIds` / `collectFailedLoaderRefs` | 区分安装自有行与树外行，并读取 Loader 失败链（[理据](../../../.agents/notes/implemented/architecture/2026-08-15-optional-profile-addons.md)） |
