...
/**
 * Harness-local compat extensions outside pi-ai's upstream compat surface.
 * These fields are not part of any `OpenAICompletionsCompat`-style gate, so
 * the drift proofs below exempt them explicitly: an upgrade to pi-ai cannot
 * gain or lose them, and the adapter reads them directly.
 */
type HarnessLocalCompatExtensions = 'mergeUserTurns'
...
export type EveryProfileFieldIsOffered = AssertNever<
  Exclude<Exclude<keyof PiAiCompatProfile, HarnessLocalCompatExtensions>, OfferedCompatField>
>
...
export type EveryProfileFieldMatchesUpstream = AssertTrue<
  Omit<PiAiCompatProfile, HarnessLocalCompatExtensions> extends Partial<Pick<UpstreamCompat, OfferedCompatField>>
    ? Partial<Pick<UpstreamCompat, OfferedCompatField>> extends Omit<PiAiCompatProfile, HarnessLocalCompatExtensions> ? true : false
    : false
>
