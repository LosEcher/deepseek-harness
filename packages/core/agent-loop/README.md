...
`shutdownDrain` is phase-aware: a model wait or pre-step returns immediately and a tool in flight waits up to the grace. A process-exiting drain writes `turn/pending` instead of cancelling, then flushes every live session. `markDraining` is loop-level — agents created after it inherit the wake gate. Resume of a `turn/pending` tail continues that turn: it closes any open step, opens the next step, and re-issues the model request from `deriveMessages()`. Publication auto-starts that continuation. A coordinated restart waits only for `hasBlockingActivity()` (tool in flight); model wait uses the pending path.
