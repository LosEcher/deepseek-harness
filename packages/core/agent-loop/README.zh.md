...
`shutdownDrain` 按阶段决策：模型等待或 pre-step 立即返回，在途工具则等到宽限。进程即将退出时的 drain 写入 `turn/pending` 而不取消，然后 flush 每个活会话。`markDraining` 是 loop 级门闩——之后创建的 agent 继承该唤醒门。恢复带 `turn/pending` 尾的会话会续跑同一回合：先闭合未结束的 step，再打开下一 step，并从 `deriveMessages()` 重发模型请求。发布后自动开始这次续跑。协调重启只等待 `hasBlockingActivity()`（工具在途）；模型等待走 pending 路径。
