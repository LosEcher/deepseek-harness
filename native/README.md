...
Native source and public packages maintained with DeepSeek Harness. The [`dsh/` workspace](dsh/README.md) contains the isolated Rust capability-provider and bridge prototype. The [`landlock-run/` workspace](landlock-run/README.md) owns the Landlock self-restrict-then-exec launcher consumed by the harness, including its architecture, three-package npm family, platform support, development workflow, and [release procedure](landlock-run/docs/release.md).
